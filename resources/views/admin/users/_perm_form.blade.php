...
<form method="POST" action="{{ route('users.permissions.update', $user) }}" id="perm-drawer-form" style="padding:4px 0;">
    ...
    @if($user->isAdmin())
        <div style="display:flex;align-items:center;gap:10px;padding:12px 14px;background:var(--accent-2);color:var(--accent-text);border-radius:var(--radius);font-size:13px;margin-bottom:18px;">
            ...
            <span><b>Admin role</b> — full access to all resources. Per-resource toggles below are for reference only.</span>
        </div>
    @endif
    ...
    <div style="display:grid;grid-template-columns:1fr repeat(4,42px);gap:6px;align-items:center;padding:0 12px 8px;border-bottom:1px solid var(--border-2);">
        <span style="font-size:11px;font-weight:600;color:var(--text-3);text-transform:uppercase;letter-spacing:.05em;">Resource</span>
        @foreach([
            ['view',    'View',    '<path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>'],
            ['edit',    'Edit',    '<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>'],
            ['delete',  'Delete',  '<polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/>'],
            ['api_key', 'API key', '<path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5"/>'],
    ...
    {{-- Groups + sites tree --}}
    @if($groups->isEmpty())
        <div style="padding:24px;text-align:center;color:var(--text-3);font-size:13px;">No groups or sites yet</div>
    @else
    ...
    @if(!$user->isAdmin())
        <div style="margin-top:14px;padding:10px 12px;background:var(--panel-2);border-radius:var(--radius);font-size:12px;color:var(--text-3);">
            <b>Tip:</b> «All resources» grants access globally. Per-group/per-site checkboxes apply only to that scope.
        </div>
    @endif
</form>
