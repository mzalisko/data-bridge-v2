...
<div class="field">
    <label class="field__label" for="pr-label-{{ $pid }}">Label</label>
    <input type="text" id="pr-label-{{ $pid }}" name="label" class="field__input" required
           placeholder="Standard package" value="{{ old('label', $p?->label) }}">
</div>

<div style="display:grid;grid-template-columns:2fr 1fr;gap:10px;">
    <div class="field">
        <label class="field__label" for="pr-amt-{{ $pid }}">Amount</label>
        <input type="number" step="0.01" min="0" id="pr-amt-{{ $pid }}" name="amount"
               class="field__input" required value="{{ old('amount', $p?->amount) }}">
    </div>
    <div class="field">
        <label class="field__label" for="pr-cur-{{ $pid }}">Currency</label>
...
<div class="field">
    <label class="field__label" for="pr-per-{{ $pid }}">Period (optional)</label>
    <input type="text" id="pr-per-{{ $pid }}" name="period" class="field__input"
           placeholder="month, year, one-time…" value="{{ old('period', $p?->period) }}">
</div>
...
<div style="display:flex;align-items:center;gap:8px;margin-top:6px;">
    ...
    <label for="pr-vis-{{ $pid }}" style="font-size:13px;color:var(--text-2);cursor:pointer;">Visible on site</label>
</div>
